...
assert_true(str_contains($index, 'Application created by Dr. Christopher B. Germann') && str_contains($index, '$appVersion = \'2.1.5\'') && str_contains($index, 'Version <?= h($appVersion) ?>') && str_contains($index, 'gmdate(\'Y\')') && str_contains($index, 'data-client-environment'), 'footer creator/version/year/client environment metadata missing');
...
assert_true(!str_contains($htaccess, 'QUERY_STRING} ^q=') && str_contains($htaccess, 'RewriteRule ^var/www/vhosts/') && str_contains($citationNetworkPhp, "request_value('search', request_value('q', ''))") && str_contains($citationNetworkPhp, 'name="search"'), 'citation search should avoid filesystem-path redirects while retaining q compatibility');
...
assert_true(str_contains($citationNetworkPhp, 'Choose the literature') && str_contains($citationNetworkPhp, 'Explore the network') && str_contains($citationNetworkPhp, 'Inspect selection') && str_contains($citationNetworkPhp, 'citation-analysis-tools') && str_contains($citationNetworkPhp, 'How to read this:') && !str_contains($citationNetworkPhp, 'data-citation-labels') && str_contains($js, 'syncCitationSelectionActions'), 'citation network workflow hierarchy, progressive disclosure, or selection action state missing');
assert_true(str_contains($citationNetworkPhp, 'data-icon="list-filter"') && str_contains($citationNetworkPhp, 'data-icon="settings"') && str_contains($css, '.citation-filter-advanced > summary:hover') && str_contains($css, '.citation-analysis-tools > summary::after') && str_contains($css, '.citation-analysis-tools[open]'), 'citation network accordion emphasis or disclosure states missing');
...
assert_true(Config::contactEmail() === 'christopher-germann@uni-wh.de' && Config::alertFromEmail() === 'noreply@psilocybin-research.com', 'contact and alert sender addresses should remain separate');
$dataProtectionNotice = file_get_contents(__DIR__ . '/../data-protection.php');
assert_true(str_contains($dataProtectionNotice, 'Config::contactEmail()') && !str_contains($dataProtectionNotice, 'published for contact purposes only') && !str_contains($dataProtectionNotice, 'Alert confirmation and digest emails'), 'data protection notice should publish the contact address without alert-sender commentary');
...
$pushReflection = new ReflectionClass($pushService);
$derToJose = $pushReflection->getMethod('derToJose');
$signingKey = openssl_pkey_new(['private_key_type' => OPENSSL_KEYTYPE_EC, 'curve_name' => 'prime256v1']);
assert_true($signingKey !== false && openssl_sign('push-signature-regression', $derSignature, $signingKey, OPENSSL_ALGO_SHA256), 'push test signature could not be generated');
$joseSignature = $derToJose->invoke($pushService, $derSignature);
assert_true(is_string($joseSignature) && strlen($joseSignature) === 64, 'DER ECDSA signature should convert to a 64-byte JOSE signature');
...
$authorVariantBase = array_merge($base, ['doi' => null, 'pubmed_id' => null]);
foreach ([
    ['Robin Carhart-Harris', 'Canonical author variant one', '10.1000/author-variant-1'],
    ['Carhart-Harris RL.', 'Canonical author variant two', '10.1000/author-variant-2'],
    ['Carhart-Harris R', 'Canonical author variant three', '10.1000/author-variant-3'],
] as [$variantName, $variantTitle, $variantDoi]) {
    assert_true($repo->upsert(array_merge($authorVariantBase, [
        'authors' => $variantName,
        'title' => $variantTitle,
        'doi' => $variantDoi,
    ])) === 'inserted', 'author-directory variant fixture insert failed');
}
$canonicalAuthors = $repo->authors('Carhart-Harris', 10);
assert_true(count($canonicalAuthors) === 1 && $canonicalAuthors[0]['name'] === 'Robin Carhart-Harris' && $canonicalAuthors[0]['count'] === 3, 'author directory should combine compatible full and abbreviated names');
foreach ([
    ['David Erritzoe', 'Canonical author diacritic one', '10.1000/author-diacritic-1'],
    ['David Erritzøe', 'Canonical author diacritic two', '10.1000/author-diacritic-2'],
] as [$variantName, $variantTitle, $variantDoi]) {
    assert_true($repo->upsert(array_merge($authorVariantBase, [
        'authors' => $variantName,
        'title' => $variantTitle,
        'doi' => $variantDoi,
    ])) === 'inserted', 'author-directory diacritic fixture insert failed');
}
$diacriticAuthors = $repo->authors('Erritzoe', 10);
assert_true(count($diacriticAuthors) === 1 && $diacriticAuthors[0]['name'] === 'David Erritzøe' && $diacriticAuthors[0]['count'] === 2, 'author directory should combine diacritic variants and retain the richer spelling');
